...
//お気に入り機能
$favorite_query = "SELECT * FROM user INNER JOIN favorite_fm ON user.no = favorite_fm.fm_no  WHERE favorite_fm.co_no = ".$_SESSION['user']."";
$favorite_result = $mysqli->query($favorite_query);
	...
	if(isset($_POST['yes'])) {
		...
		if (!$favorite_del_result) {
		print('クエリーが失敗しました。' . $mysqli->error);
		$mysqli->close();
		exit();
		}
		 //header("Location:co_favorite.php");
	}
	...
	<div class="wrapper">
		...
		<div id="main-content">
			  ...
			  <div class="container">
			  	  <div class="col-xs-12">
			  	      <ul class="cf">
			  	       ...
			  	       <li><a href="co_favorite.php" class="active"><i class="fa fa-heart" aria-hidden="true"></i> <span class="sp-none">お気に入りのFM</span></a></li>
			  	       <li><a href="co_progress_new.php"><i class="fa fa-check-square-o" aria-hidden="true"></i> <span class="sp-none">案件管理</span></a></li>
		    ...
		    <section id="user_config" class="clearfix">
			    <div class="container">
				    <h2 id="top"><i class="fa fa-heart" aria-hidden="true"></i> お気に入りのFM</h2>
				    <?php
				    while($favorite_row = $favorite_result->fetch_assoc()){
					    $fm_name = $favorite_row['name'];
					    $fm_no = $favorite_row['fm_no'];
					    $img_sub_prof_path = $favorite_row['img_sub_prof_path']; ?>
				    <div class="col-xs-12 col-sm-4 col-md-3 mb10">
		  			  <div class="box01 alpha mall-05">
		  			    <a href="<?php echo $url; ?>/prof.php?no=<?php echo $fm_no; ?>">
		  				    <p class="mb10"><img src="<?php echo $url; ?>/upload/<?php echo $img_sub_prof_path; ?>" alt="" class="img-responsive center-block" height="150px"></p>
		  				    <p class="fcolorgray"><?php echo $fm_name; ?></p>
		  				    <p class="text-right fcolorgray"><i class="fa fa-tags" aria-hidden="true"></i> カテゴリ</p>
		  				</a>
		  			  </div>
				    </div>
				    <?php

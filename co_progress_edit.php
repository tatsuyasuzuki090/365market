...
$query_project_edit = "SELECT * FROM project WHERE project_no = '$project_no'";	
$result_project_edit = $mysqli->query($query_project_edit);
$row_project_edit = $result_project_edit->fetch_assoc();
$project_no_edit = $row_project_edit['project_no'];
$project_title_edit = $row_project_edit['title'];
$project_address_edit = $row_project_edit['project_address'];
$project_datetime_edit = $row_project_edit['project_datetime'];
$project_description_edit = $row_project_edit['description'];
$project_main_img_path_edit = $row_project_edit['main_img_path'];
//$project_url_edit = $row_project_edit['project_url'];


if(isset($_POST['project'])) {
	if(empty($_POST['project_title'])){
		$project_title_error = "<div class=\"alert_none alert-warning\">案件名を入力してください</div>";
	}
	if(empty($_POST['project_address'])){
		$project_address_error = "<div class=\"alert_none alert-warning\">開催住所を入力してください</div>";
	}
	if(empty($_POST['project_description'])){
		$project_description_error =  "<div class=\"alert_none alert-warning\">説明を入力してください</div>";
	}
	if(empty($_POST['project_date'])){
		$project_date_error =  "<div class=\"alert_none alert-warning\">開催日を入力してください</div>";
	}
	
	
	$up_img = $project_main_img_path_edit;
	...
	$project_datetime_edit = $_POST['project_date'];
	$datetime_now = date("Y-m-d H:i:s");
	$datetime_number = date("YmdHis");
	$query_project_in = "UPDATE project SET title = '$project_title',description = '$project_description',project_datetime = '$project_datetime_edit',project_address = '$project_address',main_img_path = '$up_img' WHERE project_no = '$project_no'";
	...
	if(isset($result_project_in)) {  ?>
		...
		<?php } else { ?>
		<div class="alert alertfadeOut alert-danger" role="alert">エラーが発生しました。</div>
	<?php
	} ?>
<?php
} ?>
				    ...
				    <div class="col-xs-12 col-sm-9">
					    <div class="pall-10">
							...
							<table class="user_config_table">
								<tr>
			  						<th>詳細説明</th>
			  						<td><textarea  rows="10" id="textarea" class="form-control" name="project_description" placeholder="詳細説明" value="<?php echo $project_description_edit; ?>" ><?php echo $project_description_edit; ?></textarea>
				  						<?php if(isset($project_description_error)) { echo $project_description_error; }?>
			  						</td>
			  					</tr>
							</table>
							<h2 id="top">開催情報</h2>
						    <table class="user_config_table">
							    <tr>
			  						<th>タイトル</th>
			  						<td><input class="form-control" type="text" id="project_title" placeholder="案件名" name="project_title" value="<?php echo $project_title_edit; ?>">
			  						<?php if(isset($project_title_error)) { echo $project_title_error; }?>
			  						</td>
			  					</tr>
							    <tr>
			  						<th>メイン画像</th>
			  						<td class="form-inline">
				  					<img src="upload/<?php echo $project_main_img_path_edit; ?>" alt="" class="img-responsive"><br />
			  					...
			  					<tr>
			  						<th>開催場所</th>
			  						<td><input class="form-control" type="text" id="project_address" placeholder="開催場所" name="project_address" value="<?php echo $project_address_edit; ?>">
			  						<?php if(isset($project_address_error)) { echo $project_address_error; }?>
			  						</td>
			  					</tr>
			  					<tr>
			  						<th>開催日</th>
			  						<td><input class="form-control" type="datetime-local" id="project_date" placeholder="開催日" name="project_date" value="<?php if(isset($_POST['project_date'])) {echo $_POST['project_date']; } else { echo date("Y-m-d\TH:i",strtotime($project_datetime_edit)); } ?>">
			  						<?php if(isset($error['project_date'])){ echo($error['project_date']);} ?>
			  						</td>
			  					</tr>
			  					<tr>
			  						<th>URL</th>

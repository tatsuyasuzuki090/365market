...
if( isset($_SESSION['user']) != "") {
	...
	//ニュース詳細
	$news_id_detail = $_GET['id'];
	$query_news_list = "SELECT * FROM news WHERE id = '$news_id_detail'";
	$result_news_list = $mysqli->query($query_news_list);
	$row_news_list = $result_news_list->fetch_assoc();
	$news_title_list = $row_news_list['news_title'];
	$news_text_list = $row_news_list['news_text'];
	$category_list = $row_news_list['category'];
	$datetime_news_list = $row_news_list['datetime'];
	$datetime_news_list = date_a($datetime_news_list);
	
	//既読チェックデータが無ければinsert
	$query_read = "SELECT * FROM news_read WHERE no = '$news_id_detail' AND user_no = ".$_SESSION['user']."";
	$result_read = $mysqli->query($query_read);
	$row_read = $result_read->fetch_assoc();
	$user_no = $row_read['user_no'];
	$datetime_now = date("Y-m-d H:i:s");
	if ( isset($user_no) ) {
		} else {
		$query_news_in = "INSERT INTO news_read (no, user_no, category, datetime) VALUES ('$news_id_detail', ".$_SESSION['user'].", '$category_list', '$datetime_now')";
		$result_news_in = $mysqli->query($query_news_in);
  ...
  <p id="page-top"><a href="#wrap">PAGE TOP</a></p>
	...
	<div class="wrapper">
		...
		<div id="main-content">
		    ...
		    <section id="user_news" class="clearfix">
			    <div class="container">
				    <h2 id="top"><i class="fa fa-bell fa-fw" aria-hidden="true"></i> アクティビティ</h2>

					
					<div class="col-xs-12">
						<hr />
				 <h3><?php echo $news_title_list; ?></h3>
				 <?php echo $news_text_list; ?>
				 <hr />
				 <p class="text-right fs-80"><?php echo $datetime_news_list; ?><?php categry($category_list); ?></p>
				</div>
				</div>
